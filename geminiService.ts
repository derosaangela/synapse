...
import { Company, Role, Team, Contact } from "../types";
...
export const extractContactFromNaturalLanguage = async (text: string, currentCity: string): Promise<Partial<Contact>> => {
  if (!ai) return {};
  
  try {
    const response = await ai.models.generateContent({
      model: 'gemini-3-flash-preview',
      contents: `Extract contact information from this natural language description: "${text}".
      Use "${currentCity}" as the default location if one is not mentioned.
      Format the notes section as:
      Location: [Extracted Location]
      Company: [Extracted Company]
      University: [Extracted University]
      Common Ground: [Extracted Common Interests/Shared Experience]
      ...
      config: {
        ...
        responseSchema: {
          ...
          properties: {
            ...
            notes: { type: Type.STRING, description: "Structured notes following the requested template." },
            cadence: { type: Type.NUMBER, description: "Suggested keep-in-touch frequency in days (e.g., 30 for monthly, 90 for quarterly). Default to 30 if not implied." },
          },
          required: ["name", "notes"],
        },
      },
...
export const generateReachOutMessage = async (contactName: string, notes: string, cadence?: number): Promise<string> => {
  if (!ai) return "";
  
  const frequencyInfo = cadence && cadence > 0 
    ? `I aim to keep in touch with this person every ${cadence} days.` 
    : "I want to catch up with this person.";

  try {
    const response = await ai.models.generateContent({
      model: 'gemini-3-flash-preview',
      contents: `Draft a warm, professional, and personalized reach-out message for my contact ${contactName}. 
      Context notes from our previous interactions: "${notes}". 
      Relationship goal: ${frequencyInfo}
      The tone should be natural, genuine, and friendly (3-4 sentences max). 
      Reference specific details from the notes if they seem relevant for a catch-up. 
      Avoid corporate buzzwords or sounding like an automated template.`,
    });
    return response.text || "";
  } catch (error) {
    console.error("Error generating reach-out message:", error);
    return "";
  }
};
